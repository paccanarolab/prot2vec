...
def extract_proteins_representation_dict(device, model, dataset,
                                         set_label, data):
    for prot, numpy_repr in track(dataset.interpro_dict.items(),
                                  description=f'processing {set_label} set'):
        p = torch.from_numpy(
                numpy_repr.astype(np.float32)).to(device).reshape(1, -1)
        v = model.prot2vec(p)
...
def run(run_config):
    ...
    for model_class, activation in product(model_classes, activations):
        ...
        save_filename_model = os.path.join(
            config["model"]["dir_model_output"], save_name)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Generate vectors from a prot2vec model.")
